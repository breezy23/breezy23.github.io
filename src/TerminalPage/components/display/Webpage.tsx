export const Webpage = () => {
    return (
        <div>
            <div>
                You're looking at it! :D
            </div>
            <div>
                I built this site to serve as both a portfolio and a personal playground of sorts; a place to showcase
                my work and also share things related to my own hobbies and interests. It also updates my old website so
                that it can easily grow or change if I need it to.
            </div>
            <div>
                I decided to build this site in the style of a retro terminal because I enjoy the aesthetic of older technology, but I also wanted to experiment 
                building a custom UI, and work on making it very interactive.
            </div>
            <div><a
                href={'https://github.com/breezy23/breezy23.github.io'}
                target={'_blank'}
            >https://github.com/breezy23/breezy23.github.io</a></div>
        </div>
    )
}